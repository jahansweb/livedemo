...
<html>
<head>
<link rel="stylesheet" href="greenCSS/bill.css">
<title>Bill</title>
</head>
<body>
<%
 ...
 try{
	...
	PreparedStatement ps=con.prepareStatement("select sum(total) from cart where email='"+email+"' and status='bill'");
	ResultSet rs=ps.executeQuery();
	while(rs.next())
	{
		total =rs.getInt(1);
	}
	ResultSet rs1=ps.executeQuery("select * from users inner join cart where cart.email= '"+email+"' and cart.status='bill'");
	while(rs1.next())
	{
%>
...
<%
ResultSet rs2=ps.executeQuery("select * from cart inner join allmedicines where cart.pid= allmedicines.id and cart.email='"+email+"' and cart.status='bill'");
while(rs2.next())
{
	sno=sno+1;
%>  
